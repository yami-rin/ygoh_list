...
これは対戦を速めるための事前研究。MDPro3での自動入力にはまだ接続しておらず、全合法手順・全妨害応答の網羅や最善性を保証しない。

全分岐の実core探索は[探索範囲と進捗](docs/SEARCH_COVERAGE.md)に別途記録する。初動ごとの完了範囲、素材・配置・任意効果・ドロー後の未探索枝と再開コマンドを掲載。`npm run search:report`で集約し、`npm run search:verify`で元資料との一致を検査する。
