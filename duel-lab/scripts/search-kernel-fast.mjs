 ...
 * Opt-in v3: keep the actual Duel only for the immediately queued first child;
 * all siblings still replay their exact prefixes. No additional pruning occurs.
 */
...
export const implementation = Object.freeze({
  id: 'exact-first-child-duel-reuse-v1', version: VERSION, baselineVersion: 2,
  ...
  baselineFile: 'scripts/search-kernel.mjs',
  baselineSha256: '099f0170ae3f8247841bca270b30c2b5eeeb210b2385170b10738b048be0e161',
});
...
async function replayPrefix(hand, prefix, seed, {deckOrder, pruneLinkUiLoops, rootDepth}, execution) {
  ...
  try {
    for (let i = 0; i < prefix.length; i++) {
      ...
      if (result.rejected || result.pruned) return {...result, advance};
    }
    return {game, advance};
...
export async function search(options = {}) {
  ...
  if (previous && (![1, 2, VERSION].includes(previous.version) || previous.presetHash !== presetHash ||
    ...
    try {
      ...
      if (retained?.frame === frame) {
        ...
        context = await replayPrefix(hand, frame.prefix, seed,
          {deckOrder, pruneLinkUiLoops, rootDepth: prefix.length}, execution);
        game = context.game;
        replayed = context;
      }
      visited++; localNodes++;
