 ...
 * Opt-in v4: v3 first-child execution reuse plus the narrowly audited fresh
 * initial-hand Baldrake/Magnamhut normal-summon/tribute-cancel contraction.
 */
...
export const implementation = Object.freeze({
  id: 'fresh-bystial-tribute-cancel-v1', version: VERSION, baselineVersion: 3,
  ...
  baselineFile: 'scripts/search-kernel-fast.mjs',
  baselineSha256: 'ce5d28699f0cdc30adc5e29790100714e01604661a8205d444626f29cfec1aae',
});
...
const TRIBUTE_POLICY = 'fresh-initial-hand-bystial-normal-tribute-cancel-v1';
...
export function tributeAuditIdentity() {
  ...
  return {policy:TRIBUTE_POLICY,expectedPin:TRIBUTE_PIN,installedPin,
    ...
    evidence:'runtime/multi-search-depth-audit/{continuations,magnamhut-continuations}.json; core-operations.cpp:2012; core-playerop.cpp:685; preset-normal-summon-scan.json'};
}
...
function makeTributeAudit(game, hand, enabled, rootDepth) {
  ...
  return {
    ...
    before(input,index) {
      ...
      if(choice?.response.action!==0 || !virgin.has(choice.card?.code) || choice.card.controller!==game.first || choice.card.location!==2)return;
      ...
      transaction={card:choice.card.code,entry:index,stage:'tribute',state:observedLinkState(game),pending:hash(game.pending)};
    },
    after(_input,index) {
      ...
      return {kind:'cancelledNormalTribute',card:entry.card,transactionDepth:entry.entry,
...
export async function search(options = {}) {
  ...
  if (previous && (![1, 2, 3, VERSION].includes(previous.version) || previous.presetHash !== presetHash ||
  ...
  if(previous?.pruning?.tributeUiLoops?.enabled && (!pruneTributeUiLoops ||
    previous.pruning.tributeUiLoops.policy!==TRIBUTE_POLICY || previous.pruning.tributeUiLoops.expectedPin!==TRIBUTE_PIN))
  ...
  const scope = {
    ...
    tributeUi: pruneTributeUiLoops ? 'Only pinned, unique initial-hand Baldrake/Magnamhut that never left hand: exact ordinary one-tribute summon/cancel transaction; no other summon or set procedure' : 'Raw normal-tribute response histories; no Tribute UI contraction',
  };
  ...
  while (stack.length && localNodes < maxNodes && !expired()) {
    ...
    try {
      ...
      if (replayed.pruned) {
        if(replayed.pruned.kind==='cancelledNormalTribute'){
          tributeCounts.cancelledNormalTribute++;
  ...
  const result = {
    ...
    pruning: {stateMerging: false, strategic: false, exactDuplicatePrefixes: duplicates,
      tributeUiLoops:{...tributeIdentity,enabled:pruneTributeUiLoops,requested:requestedTributePruning,
        ...
        guard:'Only unique initial-hand Baldrake/Magnamhut never moved out of hand; Idle action0, HINT 3/500 + SELECT_TRIBUTE(cancelable,min=max=1,own face-up MZONE release_param=1), cancel, Idle; identical full state and pending; ancestor at/after rootPrefix'},
