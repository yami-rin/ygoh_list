...
import {search, auditTributeCancellations} from './search-kernel-tribute.mjs';
import {searchIdentity as legacyIdentity, scope as legacyScope} from './search-multi-pairs-fast.mjs';
...
export const SCHEMA = 'astra-multi-pair-search-tribute-v1';
...
const sourceFiles = ['scripts/search-multi-pairs-tribute.mjs', 'scripts/search-kernel-tribute.mjs', 'scripts/search-multi-pairs-fast.mjs', 'scripts/search-kernel-fast.mjs', 'scripts/search-multi-pairs.mjs', 'scripts/search-kernel.mjs', 'scripts/route-harness.mjs', 'scripts/patch-core.mjs', 'engine.mjs', 'prompts.mjs', 'cards.mjs', 'node_modules/ocgcore-wasm/dist/index.js'];
...
export const scope = {
  ...
  stateMerging: false,
  uiCycles: 'Only audited standard Link UI cycles and source-pinned normal-summon Tribute cancellations of untouched unique initial-hand Baldrake or Magnamhut may be contracted; other histories remain distinct',
};
...
export async function importLegacyShard({shard=0,shards=8,pairIndices,
  legacyRoot=path.join(ROOT,'runtime/multi-pair-search-fast'),
  outputRoot=path.join(ROOT,'runtime/multi-pair-search-tribute')}={}){
  ...
  try{
    ...
    for(const pair of selected){
      ...
      assert([2,3].includes(old.search.version));assert.deepEqual(old.search.hand,pair.hand);
      ...
      const next={...old,schema:SCHEMA,identity:identity(pair,123),scope,
        migration:{kind:'live-duel-to-tribute-policy-v1',sourceFile:path.relative(ROOT,sourceFile).replaceAll('\\','/'),
...
export async function runPair({pairIndex, maxNodes = 100, maxMs = 3000, maxDepth = 100,
  ...
  const directory = path.resolve(runtimeDir ?? path.join(ROOT, 'runtime/multi-pair-search-tribute', `shard-${pair.index % 8}`));
...
export async function runShard({shard = 0, shards = 8, pairIndices, maxNodesPerPair = 100,
  ...
  const directory = path.resolve(runtimeDir ?? path.join(outputRoot ?? path.join(ROOT, 'runtime/multi-pair-search-tribute'), `shard-${shard}`));
  fs.mkdirSync(directory, {recursive: true});
...
async function cli(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (numeric[flag]) options[numeric[flag]] = Number(argv[++i]);
      ...
      console.log('node scripts/search-multi-pairs-tribute.mjs --shard 0 --shards 8 --nodes-per-pair 100 --ms-per-pair 3000 --depth 100 --passes 1 [--pair-indices 0,8] [--runtime-dir PATH] [--fresh] [--import-legacy]'); return;
