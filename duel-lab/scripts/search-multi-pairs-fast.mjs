...
import {search} from './search-kernel-fast.mjs';
import {searchIdentity as legacyIdentity, scope as legacyScope} from './search-multi-pairs.mjs';
...
export const SCHEMA = 'astra-multi-pair-search-fast-v1';
...
const sourceFiles = ['scripts/search-multi-pairs-fast.mjs', 'scripts/search-kernel-fast.mjs', 'scripts/search-multi-pairs.mjs', 'scripts/search-kernel.mjs', 'scripts/route-harness.mjs', 'scripts/patch-core.mjs', 'engine.mjs', 'prompts.mjs', 'cards.mjs', 'node_modules/ocgcore-wasm/dist/index.js'];
...
function summarizePair(checkpoint, checkpointFile) {
  ...
  return {index: pair.index, id: pair.id, hand: pair.hand, names: pair.names, physicalWeight: pair.physicalWeight,
    ...
    pruning: {stateMerging: false, linkUiLoops: run.pruning.linkUiLoops.counts}};
}
...
export async function importLegacyShard({shard=0,shards=8,pairIndices,
  legacyRoot=path.join(ROOT,'runtime/multi-pair-search'),
  outputRoot=path.join(ROOT,'runtime/multi-pair-search-fast')}={}){
  ...
  try{
    ...
    for(const pair of selected){
      ...
      assert.equal(old.search.version,2);assert.deepEqual(old.search.hand,pair.hand);
      ...
      const next={...old,schema:SCHEMA,identity:identity(pair,123),scope,
        migration:{kind:'reference-to-live-duel-v1',sourceFile:path.relative(ROOT,sourceFile).replaceAll('\\','/'),
          originHash,payloadHash,from:legacyIdentity,importedAt:new Date().toISOString()}};
...
export async function runPair({pairIndex, maxNodes = 100, maxMs = 3000, maxDepth = 100,
  ...
  const directory = path.resolve(runtimeDir ?? path.join(ROOT, 'runtime/multi-pair-search-fast', `shard-${pair.index % 8}`));
...
export async function runShard({shard = 0, shards = 8, pairIndices, maxNodesPerPair = 100,
  ...
  const directory = path.resolve(runtimeDir ?? path.join(outputRoot ?? path.join(ROOT, 'runtime/multi-pair-search-fast'), `shard-${shard}`));
  fs.mkdirSync(directory, {recursive: true});
...
async function cli(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (numeric[flag]) options[numeric[flag]] = Number(argv[++i]);
      ...
      console.log('node scripts/search-multi-pairs-fast.mjs --shard 0 --shards 8 --nodes-per-pair 100 --ms-per-pair 3000 --depth 100 --passes 1 [--pair-indices 0,8] [--runtime-dir PATH] [--fresh] [--import-legacy]'); return;
