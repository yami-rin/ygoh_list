...
この最初の一巡では、短いprefixを優先する探索順により取消しを含む旧長prefixが残った。例はpair265の100入力超515件、pair286の100入力以上843件、pair215の180入力以上かつ取消し50回以上217件。

その後の [保存枝監査](MULTI_SEARCH_FRONTIER_AUDIT.md) で、６組の既存11,318枝を再生し、証明済みの取消し10,740枝を除去した。残枝は69,167、完了136組・未完197組で、元の訪問442,207・終端126,411・除外ドロー8,482と保存代表は変わらない。監査の11,318再生は探索訪問へ加算しない。縮約累計10,906件にはこの監査分を含む。
