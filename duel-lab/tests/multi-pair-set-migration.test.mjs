...
import {runPair as reference,pairs} from '../scripts/search-multi-pairs-tribute.mjs';
import {importLegacyShard,runPair,searchIdentity,shortenAuditedRoute} from '../scripts/search-multi-pairs-set.mjs';
...
test('set shortening removes mixed audited cancellations and replays every retained input',async()=>{
  ...
  try{
    ...
    const direct=finishRoute(game,{id:'set-shortening-direct'});
    // finishRoute shares the live route array, so retain an immutable baseline.
    const baseline=structuredClone(direct);
    for(const wire of [3,0,3,0]){
      const choice=game.prompt.choices.find(c=>c.card?.code===72656408&&c.response.action===wire);assert(choice);
      respond(game,{action:choice.id});assert.equal(game.prompt.type,'SELECT_TRIBUTE');respond(game,{cancel:true});
    }
    const original=finishRoute(game,{id:'set-shortening-mixed',score:1.9-game.inputs.length/1000});
    const before=hash(original),short=await shortenAuditedRoute(original);
    assert.equal(hash(original),before);assert.equal(short.shortening.removedPairs.length,4);
    assert.equal(short.steps.length,baseline.steps.length);assert.deepEqual(short.steps,baseline.steps);
    assert.equal(short.finalHash,baseline.finalHash);assert.deepEqual(short.log,baseline.log);
    assert(Math.abs(short.score-original.score-0.008)<1e-10);
    await replay(short);assert.deepEqual(await shortenAuditedRoute(short),short);
  }finally{game.close();}
});
...
test('set publication requires its own identity and cannot silently mix with reference shards',()=>{
...
test('set migration preserves real draw boundaries, history and cursors; resume cannot be rolled back by reimport',async()=>{
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'astra-set-migration-'));
  ...
  try{
    ...
    assert.equal(resumed.checkpoint.search.version,5);
  ...
  }finally{assert(path.resolve(root).startsWith(path.resolve(os.tmpdir())+path.sep+'astra-set-migration-'));fs.rmSync(root,{recursive:true,force:true});}
});
