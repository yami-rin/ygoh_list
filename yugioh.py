...
class YugiohCardScraper:
    ...
    def get_card_list_from_url(self, url: str) -> List[str]:
        ...
        try:
            ...
            # t_rowが見つからない場合、他の要素を探す
            if len(rows) == 0:
                # box_list内のカードを探す
                box_list = soup.find('div', id='card_list') or soup.find('div', class_='list_style')
                if box_list:
                    print(f"  カードリストコンテナを発見")
                    # formタグを探す（各カードがformで囲まれている場合）
                    forms = box_list.find_all('form')
                    print(f"  form要素: {len(forms)}個")

                    for form in forms:
                        cid_input = form.find('input', {'name': 'cid'})
                ...
                # それでも見つからない場合、ページ全体からcidを探す
                if not card_links:
                    all_cid_inputs = soup.find_all('input', {'name': 'cid'})
                    print(f"  ページ全体のCID入力要素: {len(all_cid_inputs)}個")

                    for cid_input in all_cid_inputs:
                        if cid_input.get('value'):
                            cid = cid_input['value']
                            # 重複を避ける
                            card_url = f"{self.base_url}/yugiohdb/card_search.action?ope=2&cid={cid}&request_locale=ja"
                            if card_url not in card_links:
                                card_links.append(card_url)
            else:
                # t_rowが見つかった場合の処理
                for row in rows:
                    ...
                    if len(card_links) % 10 == 0:
                        ...
                        if card_name_elem:
                            try:
                                ...
                                # エンコードエラーを回避
                                print(f"    ... {len(card_links)}件目: [名前表示エラー]")
    ...
    def update_cards(self, base_url: str, output_file: str = None, max_cards: int = None):
        """新しいカードのみを追加（既存カードが見つかったら終了）"""

        # 出力ファイル名を固定
        output_file = r'C:\Users\とうふ\card_manager_web\yugioh_cards_master.csv'
        ...
        print(f"既存のカード数: {len(existing_ids)}枚\n")

        cards_to_add = []
        total_checked = 0
        total_new = 0
        page = 1
        consecutive_existing = 0  # 連続して既存カードが見つかった数
        stop_threshold = 20  # この数以上既存カードが連続したら終了（20に短縮）
        ...
        with open(output_file, mode, newline='', encoding='utf-8-sig') as csvfile:
            ...
            while True:
                ...
                new_urls = []
                existing_in_page = 0
                first_card_is_existing = False  # 最初のカードが既存かどうか

                for idx, url in enumerate(card_links):
                    cid_match = re.search(r'cid=(\d+)', url)
                    card_id = cid_match.group(1) if cid_match else None
                    if card_id:
                        total_checked += 1
                        if card_id not in existing_ids:
                            new_urls.append(url)
                            consecutive_existing = 0  # 新規カードが見つかったらリセット
                        else:
                            existing_in_page += 1
                            consecutive_existing += 1
                            # 最初のカード（最新カード）が既存の場合
                            if idx == 0:
                                first_card_is_existing = True

                print(f"  新しいカード: {len(new_urls)}/{len(card_links)}枚")
                print(f"  既存カード: {existing_in_page}枚 (連続: {consecutive_existing}枚)")

                # 新規カードがある場合は処理を続行
                if new_urls:
                    print(f"新規カード {len(new_urls)}枚の詳細を取得します...")
                    # バッチ処理で並列取得
                    ...
                    # 新規カードがない場合の終了判定
                    # 最初のカード（最新カード）が既存の場合は即座に終了
                    if first_card_is_existing and page == 1:
                        print(f"\n最新のカードが既に登録済みのため、更新を終了します。")
                        break

                    # ページ内の全カードが既存の場合も終了
                    if existing_in_page == len(card_links) and len(card_links) > 0:
                        print(f"\nページ {page} の全カードが既存のため、更新を終了します。")
                        break

                    # 連続して既存カードが見つかった場合は終了
                    if consecutive_existing >= stop_threshold:
                        print(f"\n{stop_threshold}枚以上の既存カードが連続で見つかったため、更新を終了します。")
                        break
...
def main():
    ...
    # ユーザーが指定したURL (100件ずつ取得、全ページ、リリース日でソート)
    # sess=1に変更（セッションエラーの可能性）

    base_url = "https://www.db.yugioh-card.com/yugiohdb/card_search.action?ope=1&sess=1&rp=100&stype=1&link_m=2&othercon=2&releaseYStart=1999&releaseMStart=1&releaseDStart=1&sort=21"
    ...
    # デフォルトで更新モード（新規カードのみ追加）
    print("【更新モード】新しいカードのみを追加します。")

    # 常にupdate_cardsを使用して、既存カードはスキップ
    output_file = 'yugioh_cards_master.csv'  # 固定ファイル名

    if args.test:
        print("テストモード: 最大10枚まで処理")
        scraper.update_cards(base_url, output_file=output_file, max_cards=10)
    elif args.max:
        print(f"最大 {args.max} 枚まで処理")
        scraper.update_cards(base_url, output_file=output_file, max_cards=args.max)
    else:
        scraper.update_cards(base_url, output_file=output_file)
