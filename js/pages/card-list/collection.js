...
import { applyTheme as applySharedTheme } from '../../shared/theme.js';
import { escapeHtml as escapeHtmlShared, getModal, setLoading, showToast } from '../../shared/ui.js';
import {
    applyActionCode,
    createUserWithEmailAndPassword,
    deleteUser,
    initializeCardListAuth,
    onAuthStateChanged,
    sendEmailVerification,
    signInWithEmailAndPassword,
    signOut,
} from './auth.js';
import { createDefaultGamificationData } from './gamification.js';
import { createRarityBadgeRenderer } from './render.js';
import { createSearchNormalizer, getReadingForSort as getReadingForSortFromMap } from './search.js';
import { applyCardListTheme } from './settings.js';

export function initializeCardList() {
        const auth = initializeCardListAuth();
        ...
        let cardIdToDetailsMap = new Map(); // Reverse map: cardId → details
        const getReadingForSort = (name) => getReadingForSortFromMap(cardDetailsMap, name);
        ...
        const modalElements = [
            ...
            'how-to-use-modal', 'settings-modal', 'tag-management-modal', 'alias-management-modal',
            'bulk-tag-modal', 'bulk-quantity-modal', 'analysis-modal', 'point-shop-modal',
        ...
        const rarityBadgeCellHtml = createRarityBadgeRenderer(escapeHtml);
                    ...
                    if (isMobile) {
                        ...
                        rows.push(`
                            ...
                            <td class="text-nowrap">
                                ...
                                ${showInlineQtyBtn ? `<button class="btn btn-outline-secondary btn-sm qty-plus-btn" data-id="${escapeHtml(card.id)}" style="padding:0 0.4rem;line-height:1.2;">+</button>` : ''}
                            </td>
                            <td>
        ...
        const updatePaginationControls = () => {
            ...
            // 検索結果サマリー更新
            const summaryEl = document.getElementById('search-result-summary');
            if (summaryEl) {
                const allKinds = getCurrentCollection().length;
                const filteredKinds = filteredCardCollection.length;
                const totalQty = filteredCardCollection.reduce((s, c) => s + (parseInt(c.data['枚数']) || 0), 0);
                document.getElementById('search-result-kinds').textContent = filteredKinds;
                document.getElementById('search-result-total').textContent = totalQty;
                document.getElementById('search-result-all-kinds').textContent = allKinds;
                summaryEl.style.display = filteredKinds < allKinds ? 'block' : 'none';
            }
        };
            ...
            try {
                ...
                for (const card of collection) {
                    if (card.data['名前'] === name &&
                        ...
                        if (cardQuantity > quantity) {
                            ...
                            if (isOnlineUser()) {
                                await api.updateCard(card.id, { '枚数': card.data['枚数'] });
                            }
                ...
                if (recentCardsHistory) {
                    try {
                        ...
                        // Remove the item from history (match by timestamp + name for reliable identification)
                        recentHistory = recentHistory.filter(item =>
                            !(item.timestamp === timestamp && item.name === name));
        ...
        const updateRarityChart = (distribution, total) => {
            ...
            // Rarity order from custom settings or default
            const defaultChartRarityOrder = ['N', 'NR', 'P', 'M', 'R', 'SR', 'M+SR', 'P+SR', 'UR', 'P+UR', 'UL', 'GR', 'PG', 'CR', 'HR', 'SE', 'EXSE', 'GSE', 'P+SE', 'PSE', 'QCSE', '20SE', '10000th', '25SE', 'GMR'];
            const rarityOrder = JSON.parse(localStorage.getItem('customRarityOrder') || 'null') || defaultChartRarityOrder;
            const rarityColors = {
                'N': '#868e96', 'NR': '#95a8b1', 'P': '#339af0', 'M': '#20c997',
                'R': '#51cf66', 'SR': '#ffd43b', 'M+SR': '#fab005', 'P+SR': '#ff922b',
                'UR': '#ff6b6b', 'P+UR': '#f06595', 'UL': '#cc5de8', 'GR': '#845ef7',
                'PG': '#7950f2', 'CR': '#5c7cfa', 'HR': '#4c6ef5', 'SE': '#748ffc',
                'EXSE': '#91a7ff', 'GSE': '#dbe4ff', 'PSE': '#e7f5ff', 'QCSE': '#a5d8ff',
                '20SE': '#b197fc', '10000th': '#e599f7', '25SE': '#ffa94d',
                'GMR': '#ffd700', 'default': '#868e96'
            };
            const fallbackColors = [
                '#22b8cf', '#15aabf', '#1098ad', '#0c8599',
                '#94d82d', '#82c91e', '#74b816', '#66a80f',
                '#fab005', '#fd7e14', '#f76707', '#e8590c',
            ];
            let fallbackIdx = 0;

            // Build index map for fast lookup (case-insensitive)
            const orderMap = new Map();
            rarityOrder.forEach((r, i) => orderMap.set(r.toUpperCase(), i));

            // Sort distribution entries by custom rarity order
            const entries = Object.entries(distribution)
                .filter(([, count]) => count > 0)
                .sort((a, b) => {
                    const idxA = orderMap.has(a[0].toUpperCase()) ? orderMap.get(a[0].toUpperCase()) : 9999;
                    const idxB = orderMap.has(b[0].toUpperCase()) ? orderMap.get(b[0].toUpperCase()) : 9999;
                    return idxA - idxB;
                });
            ...
            entries.forEach(([rarity, count]) => {
                ...
                let color = rarityColors[rarity] || rarityColors[rarity.toUpperCase()];
                if (!color) {
                    // Try matching against order keys
                    for (const [key, val] of Object.entries(rarityColors)) {
                        if (key.toUpperCase() === rarity.toUpperCase()) { color = val; break; }
                    }
                }
                if (!color) {
        ...
        const normalizeForSearch = createSearchNormalizer(decodeHtmlEntities);
        ...
        const matchCardType = (det, type) => {
            ...
            if (type === '通常') return det.cardType && det.cardType.split('／').includes('通常');
            return det.cardType && det.cardType.includes(type);
        };

        // Resolve card details: customCardId → ID map → name map → linkedDetails
        const resolveCardDetails = (d) => {
            if (d.customCardId) {
                const det = cardIdToDetailsMap.get(d.customCardId);
                if (det) return det;
            }
            const cardName = d['名前'] || '';
            if (cardDetailsMap.has(cardName)) return cardDetailsMap.get(cardName);
            return d.linkedDetails || null;
        };
        ...
        const applySearchAndFilter = () => {
            ...
            if (sN) {
                ...
                if (userAliasMap.has(searchKey)) {
                    possibleCardNames.push(...userAliasMap.get(searchKey));
                }
            ...
            filteredCardCollection = getCurrentCollection().filter(c => {
                ...
                const details = resolveCardDetails(d);
        ...
        const isOnlineUser = () => currentUser?.uid && currentUser.uid !== 'local_user';
        ...
        const loadAllAliases = async () => {
            if (isOnlineUser()) {
                ...
                const saved = localStorage.getItem('cardAliases');
                if (saved) {
                    try {
                        allAliases = JSON.parse(saved);
                    } catch (e) {
                        allAliases = [];
                    }
                } else {
        ...
        const renderAliasManagementList = () => {
            const container = document.getElementById('existing-aliases-list');
            if (allAliases.length === 0) {
                container.innerHTML = '<p class="text-muted">登録されたエイリアスはありません。</p>';
                return;
            }
            container.innerHTML = '';
            allAliases.forEach(a => {
                ...
                div.innerHTML = `
                    <div>
                        <span class="badge bg-secondary me-1">${escapeHtml(a.alias)}</span>
                        <i class="bi bi-arrow-right mx-1"></i>
                        <span>${escapeHtml(a.cardName)}</span>
                    </div>
                    <button class="btn btn-outline-danger btn-sm delete-alias-btn" data-id="${escapeHtml(a.id)}">削除</button>
                `;
                container.appendChild(div);
            });
        };

        const populateAliasCardNameDatalist = () => {
            const datalist = document.getElementById('alias-card-name-list');
            datalist.innerHTML = '';
            // Use unique card names from cardReadingMap values and current collection
            const names = new Set();
            for (const cards of cardReadingMap.values()) {
                cards.forEach(n => names.add(n));
            }
            getCurrentCollection().forEach(c => {
                if (c.data['名前']) names.add(c.data['名前']);
            });
            const sorted = [...names].sort();
            // Limit to prevent huge datalists
            sorted.slice(0, 2000).forEach(name => {
        ...
        document.getElementById('add-alias-form').addEventListener('submit', async (e) => {
            ...
            const cardNameInput = document.getElementById('new-alias-card-name');
            const alias = aliasInput.value.trim();
            const cardName = cardNameInput.value.trim();
            if (!alias || !cardName) return;

            // Check for duplicate alias
            if (allAliases.some(a => a.alias === alias)) {
                showNotification(`エイリアス「${alias}」は既に登録されています。`, 'warning');
                return;
            }

            try {
                if (isOnlineUser()) {
                    const created = await api.createAlias(alias, cardName);
                    allAliases.push(created);
                } else {
                    const newAlias = { id: `local_${Date.now()}_${Math.random()}`, alias, cardName };
                    allAliases.push(newAlias);
                    localStorage.setItem('cardAliases', JSON.stringify(allAliases));
                }
                buildUserAliasMap();
                renderAliasManagementList();
                aliasInput.value = '';
                cardNameInput.value = '';
                showNotification(`エイリアス「${alias}」→「${cardName}」を登録しました。`, 'success');
            } catch (err) {
                console.error('Failed to create alias:', err);
                showNotification('エイリアスの登録に失敗しました。', 'danger');
            }
        });

        document.getElementById('existing-aliases-list').addEventListener('click', async (e) => {
            const btn = e.target.closest('.delete-alias-btn');
            if (!btn) return;
            const aliasId = btn.dataset.id;
            const alias = allAliases.find(a => a.id === aliasId);
            if (!alias) return;

            if (!confirm(`エイリアス「${alias.alias}」を削除しますか？`)) return;

            try {
                if (isOnlineUser()) {
                    await api.deleteAlias(aliasId);
                }
                allAliases = allAliases.filter(a => a.id !== aliasId);
                if (!isOnlineUser()) {
                    localStorage.setItem('cardAliases', JSON.stringify(allAliases));
                }
                buildUserAliasMap();
                renderAliasManagementList();
                showNotification('エイリアスを削除しました。', 'success');
            } catch (err) {
                console.error('Failed to delete alias:', err);
                showNotification('エイリアスの削除に失敗しました。', 'danger');
            }
        });
        ...
        const saveTagsOrder = async () => {
            if (isOnlineUser()) {
                try {
                    await api.reorderTags(allTags.map(tag => ({ id: tag.id, order: tag.order })));
                } catch (error) {
                }
            } else {
        ...
        document.getElementById('add-tag-form').addEventListener('submit', async (e) => {
            ...
            for (const newTagName of newTagNames) {
                if (allTags.some(tag => tag.name === newTagName)) {
                    ...
                    if (isOnlineUser()) {
                        const tag = await api.createTag(newTagName, newOrder);
                        allTags.push({ id: tag.id, name: tag.name, order: tag.order });
                    } else {
            ...
            if (e.target.closest('.edit-tag-name-btn')) {
                ...
                if (newName && newName.trim() && newName !== oldName) {
                    ...
                    if (tagIndex !== -1) {
                        ...
                        if (isOnlineUser()) {
                            // API handles renaming across all cards server-side
                            await api.updateTag(tagId, { name: trimmedName });
                        } else {
                            localStorage.setItem('tagsOrder', JSON.stringify(allTags));
                        }
...
このタグは全てのカードからも削除されます。`)) return;
                ...
                try {
                    if (isOnlineUser()) {
                        // API handles removing tag from all cards server-side
        ...
        // listType helper (kept for local-mode branching)
        const getListType = (listType = 'collection') => listType;
        ...
        // '20th' → '20SE' データマイグレーション（一度だけ実行）
        const migrateRarity20th = (cards) => {
            cards.forEach(card => {
                if (card.data && card.data['レアリティ'] === '20th') {
                    card.data['レアリティ'] = '20SE';
                }
            });
        };
        ...
        // Metadata updates are now handled server-side automatically
        const updateMetadataTimestamp = async () => {
            // No-op: D1 API updates sync_metadata on every write
        };

        // Deletion tracking is handled server-side
        const trackDeletedId = async (cardId, colName) => {
            // No-op: D1 API tracks deletions automatically
        };
        ...
        // Force sync from API (ignores cache)
        const forceSyncFromFirebase = async () => {
            if (!currentUser || currentUser.uid === 'local_user') return;

            showLoading(true);
            console.log('Force syncing from API...');

            try {
                const [cards, wishlist] = await Promise.all([
                    api.getCards('collection'),
                    api.getCards('wishlist'),
                ]);
                cardCollection = cards.map(c => ({ id: c.id, data: c }));
                wishlistCollection = wishlist.map(c => ({ id: c.id, data: c }));
        ...
        const loadCollectionFromFirestore = async () => {
            ...
            if (cached && cached.cachedAt) {
                ...
                if (!needsSync) {
                    ...
                    migrateRarity20th(cardCollection);
                    migrateRarity20th(wishlistCollection);
                    updateTotalCounts();
                    applySearchAndFilter();
                    showLoading(false);
                ...
                console.log('Remote data is newer, fetching from API...');
            }
        ...
        const updateAnalysisData = () => {
            ...
            collection.forEach(card => {
                ...
                const details = resolveCardDetails(card.data);
        ...
        const updateUserRankingData = async () => {
            ...
            try {
                ...
                if (totalCards === 0 && uniqueCards === 0 && currentStreak === 0 && totalPoints === 0) {
                    ...
                    try {
                        await api.updateRanking({ totalCards: 0, uniqueCards: 0, currentStreak: 0, totalPoints: 0 });
                    } catch (error) {
                        // Ignore error
                    }
                    return;
...
- メールテンプレートのアクションURL設定`;
            ...
            document.getElementById('bulk-tag-save-btn').onclick = async () => {
                ...
                try {
                    for (const cardId of selectedCardIds) {
                        ...
                        if (isOnlineUser()) {
                            await api.updateCard(cardId, { tags: updatedTags });
                        }
                        card.data.tags = updatedTags;
                    }

                    const processedCount = selectedCardIds.size;
                    bulkTagModal.hide();
                    selectedCardIds.clear();
                    updateBulkEditControls();
                    renderTable();
                    alert(`${processedCount}枚のカードのタグを${operation === 'add' ? '追加' : '削除'}しました。`);
                } catch (error) {
                    alert('エラーが発生しました。');
                } finally {
                    showLoading(false);
        ...
        document.getElementById('alias-management-btn').addEventListener('click', () => {
            renderAliasManagementList();
            populateAliasCardNameDatalist();
            aliasManagementModal.show();
        });
        ...
        document.getElementById('bulk-quantity-btn').addEventListener('click', () => {
            ...
            document.getElementById('bulk-quantity-modal-desc').textContent =
                `${selectedCardIds.size}件のカードの枚数を変更します。`;
            document.getElementById('bulk-qty-value').value = 1;
            document.querySelector('input[name="bulk-qty-mode"][value="set"]').checked = true;
            bulkQuantityModal.show();
        });

        document.getElementById('bulk-qty-save-btn').addEventListener('click', async () => {
            const mode = document.querySelector('input[name="bulk-qty-mode"]:checked').value;
            const n = parseInt(document.getElementById('bulk-qty-value').value, 10);
            if (isNaN(n) || n < 1) { alert('1以上の整数を入力してください。'); return; }
            showLoading(true);
            try {
                const collection = getCurrentCollection();
                for (const cardId of selectedCardIds) {
                    const card = collection.find(c => c.id === cardId);
                    if (!card) continue;
                    const cur = parseInt(card.data['枚数']) || 1;
                    const newQty = mode === 'set' ? n
                                 : mode === 'add' ? cur + n
                                 : Math.max(1, cur - n);
                    if (isOnlineUser()) await api.updateCard(cardId, { '枚数': newQty });
                    card.data['枚数'] = newQty;
                }
                const count = selectedCardIds.size;
                bulkQuantityModal.hide();
                selectedCardIds.clear();
                updateBulkEditControls();
                updateTotalCounts();
                applySearchAndFilter();
        ...
        document.getElementById('bulk-delete-btn').addEventListener('click', async () => {
            ...
            try {
                ...
                applySearchAndFilter();
                alert(`${deletedCount}枚のカードを削除しました。`);
            } catch (error) {
            ...
            try {
                ...
                for (const cardId of selectedCardIds) {
                    ...
                    try {
                        ...
                        // Update via API
                        if (isOnlineUser()) {
                            await api.updateCard(cardId, { '型番': setCode });
                        }
                        // Update local data
        ...
        addForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const newCard = {
                    '名前': decodeHtmlEntities(formData.get('name')),
                    '型番': decodeHtmlEntities(formData.get('set_code')),
                    'レアリティ': decodeHtmlEntities(formData.get('rarity')),
            ...
            updateRegistrationHistoryButton();
                ...
                const dbOperation = async () => {
                    if (isOnlineUser()) {
                        // Server-side dedup: API handles duplicate check + increment
                        const result = await api.addCard(addType, newCard);
                        if (result.merged) {
                            // Was merged with existing card
                            const localCard = targetCollection.find(c => c.id === result.card.id);
                            if (localCard) localCard.data['枚数'] = result.card['枚数'];
                        } else {
                            // New card created
                            targetCollection.push({ id: result.card.id, data: result.card });
                            if (currentListType === 'collection') {
                                gamificationData.cardAdditionDates[result.card.id] = new Date().toISOString();
                                saveGamificationData();
                            }
                        }
                    } else { // ローカルモード
        ...
        cardTableBody.addEventListener('click', async (e) => {
            ...
            if (target.classList.contains('qty-plus-btn') || target.classList.contains('qty-minus-btn')) {
                const cardId = target.dataset.id;
                const collection = getCurrentCollection();
                const card = collection.find(c => c.id === cardId);
                if (!card) return;
                const delta = target.classList.contains('qty-plus-btn') ? 1 : -1;
                const newQty = Math.max(1, (parseInt(card.data['枚数']) || 1) + delta);
                // DOM即時更新（re-renderなし）
                card.data['枚数'] = newQty;
                const span = cardTableBody.querySelector(`.qty-display[data-id="${cardId}"]`);
                ...
                try {
                    ...
                    // ロールバック
                    card.data['枚数'] = newQty - delta;
                    if (span) span.textContent = card.data['枚数'];
                    alert('枚数の保存に失敗しました。');
                }
                return;
            }
            ...
            if (target.classList.contains('delete-btn')) {
                ...
                if (card && confirm(`「${card.data.名前}」を所持カードに移動しますか？`)) {
                    if (isOnlineUser()) {
                        ...
                        const result = await api.addCard('collection', card.data);
                        cardCollection.push({ id: result.card.id, data: result.card });
                        gamificationData.cardAdditionDates[result.card.id] = new Date().toISOString();
                        saveGamificationData();
        ...
        document.getElementById('save-edit-btn').addEventListener('click', async () => {
            ...
            if (isOnlineUser()) {
                await api.updateCard(cardId, { '枚数': updatedData.枚数, tags: updatedData.tags });
            }
        ...
        // Fix HTML entities in existing card data
        document.getElementById('fix-html-entities-btn').addEventListener('click', async () => {
            const STRING_FIELDS = ['名前', '型番', 'レアリティ'];
            const allCollections = [
                { list: cardCollection, type: 'collection' },
                { list: wishlistCollection, type: 'wishlist' }
            ];
            let fixedCount = 0;
            const resultEl = document.getElementById('fix-html-entities-result');
            resultEl.textContent = '修正中...';

            for (const { list, type } of allCollections) {
                for (const card of list) {
                    const updates = {};
                    for (const field of STRING_FIELDS) {
                        const original = card.data[field];
            ...
            resultEl.textContent = fixedCount > 0 ? `${fixedCount}件修正しました` : '修正対象なし';
        });
            ...
            try {
                ...
                if (importedData.collection.length > 0) {
                    if (isOnlineUser()) {
                        ...
                        const cardsToImport = importedData.collection.map(card => {
                        ...
                        // Server-side batch import with dedup
                        await api.batchImportCards('collection', cardsToImport);
                    } else { // ローカルモード
                ...
                if (importedData.wishlist.length > 0) {
                    if (isOnlineUser()) {
                        const wishlistToImport = importedData.wishlist.map(card => {
                            if (card.タグ && typeof card.タグ === 'string') {
                                card.tags = card.タグ.split(';').filter(t => t.trim());
                                delete card.タグ;
                            }
                        ...
                        await api.batchImportCards('wishlist', wishlistToImport);
                    } else { // ローカルモード
                ...
                // Reload if using API, otherwise just refresh display
                if (isOnlineUser()) {
            ...
            try {
                ...
                for (const card of cardCollection) {
                    ...
                    if (match) {
                        ...
                        // Update the card via API or local storage
                        if (isOnlineUser()) {
                            await api.updateCard(card.id, { linkedDetails: updatedData.linkedDetails });
                        }
        ...
        inlineQtyBtnToggle.checked = localStorage.getItem('inlineQtyBtn') !== 'false';
        ...
        inlineQtyBtnToggle.addEventListener('change', (e) => {
            localStorage.setItem('inlineQtyBtn', e.target.checked);
            renderTable();
        });
        ...
        const applyTheme = () => applyCardListTheme(darkModeToggle);
            ...
            try {
                ...
                if (cleanupCount > 0) {
                    ...
                    for (const { card, listType } of updatedCards) {
                        if (isOnlineUser()) {
                            await api.updateCard(card.id, { tags: card.data.tags });
                        }
                    }
...
• 未定
        ...
        */
        ...
        document.getElementById('fetch-cards-btn').addEventListener('click', async () => {
            ...
            // Extract locale from URL (e.g. request_locale=cn → 'cn')
            try {
                bulkImportLocale = new URL(url).searchParams.get('request_locale') || 'ja';
            } catch (e) {
                bulkImportLocale = 'ja';
            }
            ...
            try {
                ...
                const loadCardImage = async (cardId, imgElement, locale = 'ja') => {
                    try {
                        console.log(`Loading image for card ${cardId} (locale=${locale})...`);

                        // First, get card details to obtain the enc token
                        const detailUrl = `https://ygoh-list.onrender.com/card-detail?cid=${cardId}&locale=${locale}`;
                ...
                bulkImportCards.forEach((card, index) => {
                    ...
                    const loadWithRetry = async (retries = 3, delay = 2000) => {
                        for (let i = 0; i < retries; i++) {
                            try {
                                const detailData = await loadCardImage(card.id, imgElement, bulkImportLocale);
        ...
        bulkRegisterBtn.addEventListener('click', async (event) => {
            ...
            // Phase 1: Collect cards with quantity > 0 and ensure reprints are loaded
            const pendingCards = [];
                ...
                try {
                    ...
                    if (quantity > 0) {
                        ...
                        if (!cardData.reprints) {
                            try {
                                const detailUrl = `https://ygoh-list.onrender.com/card-detail?cid=${cardData.id}&locale=${bulkImportLocale}`;
                        ...
                        pendingCards.push({ cardData, rarity, quantity });
                    }
                } catch (error) {
            ...
            // Phase 2: Detect the common set prefix from reprints
            // When importing from a specific pack, most cards share a common set prefix (e.g. "AGOV")
            const prefixCardCount = {};
            for (const { cardData } of pendingCards) {
                if (cardData.reprints) {
                    const seenPrefixes = new Set();
                    for (const r of cardData.reprints) {
                        const prefix = r.setCode.split('-')[0];
                        if (prefix && !seenPrefixes.has(prefix)) {
                            seenPrefixes.add(prefix);
                            prefixCardCount[prefix] = (prefixCardCount[prefix] || 0) + 1;
                        }
                    }
                }
            }
            // Find the prefix that appears in the most cards
            let targetSetPrefix = '';
            let maxPrefixCount = 0;
            for (const [prefix, count] of Object.entries(prefixCardCount)) {
                if (count > maxPrefixCount) {
                    maxPrefixCount = count;
                    targetSetPrefix = prefix;
                }
            }
            console.log(`Detected target set prefix: "${targetSetPrefix}" (appears in ${maxPrefixCount}/${pendingCards.length} cards)`);

            // Phase 3: Build selectedCards with prioritized set code matching
            for (const { cardData, rarity, quantity } of pendingCards) {
                let number = '';
                if (cardData.reprints && cardData.reprints.length > 0) {
                    let match = null;
                    if (targetSetPrefix) {
                        // Priority 1: Same rarity + same set prefix
                        match = cardData.reprints.find(r =>
                            r.rarity === rarity && r.setCode.startsWith(targetSetPrefix + '-')
                        );
                        // Priority 2: Same set prefix, any rarity
                        if (!match) {
                            match = cardData.reprints.find(r =>
                                r.setCode.startsWith(targetSetPrefix + '-')
                            );
                        }
                    }
                    // Priority 3: Same rarity, any set
                    if (!match) {
                        match = cardData.reprints.find(r => r.rarity === rarity);
                    }
                    // Priority 4: First reprint (last resort)
                    if (!match) {
                        match = cardData.reprints[0];
                    }
                    number = match.setCode;
                }
            ...
            // Read bulk tags input
            const bulkTagsInput = document.getElementById('bulk-import-tags').value;
            const bulkTags = bulkTagsInput ? bulkTagsInput.split(',').map(t => t.trim()).filter(t => t) : [];
            ...
            try {
                for (const card of selectedCards) {
                    try {
                        ...
                        const cardTags = [...new Set([...(card.tags || []), ...bulkTags])];

                        // Server-side dedup via API
                        const newCard = {
                            '名前': card.name,
                            '型番': cardNumber,
                            'レアリティ': cardRarity,
                            '枚数': card.quantity,
                            tags: cardTags,
                            customCardId: card.id || null,
                            cardLang: bulkImportLocale !== 'ja' ? bulkImportLocale : null,
                        };
                        const result = await api.addCard('collection', newCard);
                        if (!result.merged) {
                            gamificationData.cardAdditionDates[result.card.id] = new Date().toISOString();
                        }
        ...
        document.getElementById('delete-account-btn').addEventListener('click', async () => {
            ...
            showLoading(true);
            try {
                // Delete all user data via single API call
        ...
        document.getElementById('search-db-btn').addEventListener('click', async () => {
            ...
            // Check user-defined aliases
            if (userAliasMap.has(searchKeyLower)) {
                possibleCardNames.push(...userAliasMap.get(searchKeyLower));
            }
            for (const [alias, cards] of userAliasMap) {
                if (alias.includes(searchKeyLower)) {
                    possibleCardNames.push(...cards);
                }
            }
            ...
            if (matchingCards.length > 0) {
                ...
                // Sort by name (50音順) using reading from master CSV
                matchingCards.sort((a, b) =>
                    getReadingForSort(a.data['名前'] || '').localeCompare(getReadingForSort(b.data['名前'] || ''), 'ja')
                );
            ...
            showLoading(true);
            document.getElementById('search-db-btn').disabled = true;
            // Resolve alias to official card name for DB search
            let dbSearchKeyword = searchKeyword;
            const aliasKey = searchKeyword.toLowerCase();
            if (userAliasMap.has(aliasKey)) {
                const aliasNames = userAliasMap.get(aliasKey);
                if (aliasNames.length === 1) {
                    dbSearchKeyword = aliasNames[0];
                }
                // Multiple matches: use first one (exact alias match takes priority)
                else if (aliasNames.length > 1) {
                    dbSearchKeyword = aliasNames[0];
                }
            }
            const searchUrl = `https://www.db.yugioh-card.com/yugiohdb/card_search.action?ope=1&sess=1&rp=100&mode=&sort=1&keyword=${encodeURIComponent(dbSearchKeyword)}&stype=1&ctype=&othercon=2&starfr=&starto=&pscaleto=&linkmarkerfr=&linkmarkerto=&link_m=2&atkfr=&atkto=&deffr=&defto=&releaseDStart=1&releaseMStart=1&releaseYStart=1999&releaseDEnd=&releaseMEnd=&releaseYEnd=`;
            ...
            try {
                ...
                showLoading(false);
                document.getElementById('search-db-btn').disabled = false;
            }
        });
