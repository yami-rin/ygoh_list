...
export async function renderExport(canvas, state, loadImage, signal) {
    const pad = 32, labelWidth = 150, cardWidth = 100, artHeight = 146, cellHeight = 190, gap = 12;
    ...
    const heights = state.tierState.map(cards => Math.max(106, Math.ceil(cards.length / columns) * cellHeight + 24));
    ...
    ctx.fillStyle = '#a8cfbf'; ctx.font = '600 13px sans-serif'; ctx.fillText('YGO CARD TOOLS / CUSTOM BANLIST', pad, 25);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 32px sans-serif';
    wrapText(ctx, state.title.trim() || 'マイ・バンリスト', width - pad * 2, 1).forEach(line => ctx.fillText(line, pad, 54));
    ctx.fillStyle = '#c5d5d6'; ctx.font = '14px sans-serif'; ctx.fillText(`${names.length}枚 · カスタムレギュレーション`, pad, 105);
    let y = 166;
    for (let index = 0; index < state.tierConfig.length; index++) {
        ...
        cards.forEach((name, cardIndex) => {
            ...
            ctx.fillStyle = '#183039'; ctx.font = '11px sans-serif';
            wrapText(ctx, name, cardWidth, 2).forEach((line, offset) => ctx.fillText(line, x, top + artHeight + 5 + offset * 15));
        });
        y += rowHeight + 10;
    }
    ctx.fillStyle = '#687b82'; ctx.font = '12px sans-serif';
    ctx.fillText('作成: YGO バンリストエディター  ·  公式の禁止・制限リストではありません', pad, height - 32);
